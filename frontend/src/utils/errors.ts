export function localizedError(message: string, t: (key: string) => string) {
  ...
  const candidates = [
    code && `error_${code}`,
    lower.includes('auth_error') && 'error_AUTH_ERROR',
    lower.includes('unauthorized') && 'error_AUTH_ERROR',
    lower.includes('api key') && 'error_AUTH_ERROR',
    lower.includes('login credential') && 'error_AUTH_ERROR',
    ...
    lower.includes('card code') && lower.includes('expired') && 'error_CARD_EXPIRED',
