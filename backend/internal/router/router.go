...
func New(cfg config.Config, services Services) *gin.Engine {
	...
	adminAPI := v1.Group("/admin")
	{
		...
		protected.POST("/providers/:provider/countries/sync", catalogHandler.SyncProviderCountries)
		protected.GET("/providers/:provider/services", catalogHandler.ProviderServices)
		protected.POST("/providers/:provider/services/sync", catalogHandler.SyncProviderServices)
